#!/usr/bin/env python3
"""Per-checkpoint GPU forward-pass for CFR analysis.
...
Use ``rcp/submit_forward_pass.sh`` to fan out across all
checkpoints.
...
Usage:
    python3 analysis/02_forward_pass.py --experiment davani_lambda10 --seed 42
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="GPU forward-pass for one CFR-evaluation checkpoint."
    )
    ...
    if not swap_manifest_path.exists():
        logger.error(
            ...
            "Run analysis/01_build_swap_manifest.py first.",
            swap_manifest_path,
        )
        return 2
    ...
    logger.info("Done — CFR-evaluation forward-pass cache complete for %s seed=%d",
                experiment, seed)
    return 0
