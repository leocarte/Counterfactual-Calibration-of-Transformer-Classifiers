# Direction 1 — Counterfactual flip-rate as the loss-aligned metric
...
```bash
bash analysis/direction1_rcp/submit_forward_pass.sh
# Or: bash analysis/direction1_rcp/submit_forward_pass.sh smoke   # one job for sanity
```
...
cat analysis/direction1/results/main_table_cfr.md
cat analysis/direction1/results/ts_sanity_check.csv
```
...
- `09_write_report.py` — REPORT.md (08 reserved for the cross-dataset transfer
  block that depends on Yasmin's CSVs and is not yet implemented)
