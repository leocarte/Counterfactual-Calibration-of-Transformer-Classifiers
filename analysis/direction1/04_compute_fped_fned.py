...
Usage:
    python3 analysis/direction1/04_compute_fped_fned.py
"""
...
def _compute_fped_fned(
    ...
    # (The previous implementation incorrectly reported max−min, i.e. the
    # range; that is "FP/FN gap", not Dixon FPED/FNED.)
...
def _build_main_table_md(summary: pd.DataFrame, output_path: Path) -> None:
    lines = ["# Direction-1 main table — FPED / FNED (Dixon AIES 2018)\n",
...
def main():
    ...
    # Write to a *new* file so the previous (incorrect, range-based) parquet
    # is preserved for direct comparison. Downstream scripts
    # (06_run_significance_suite.py, 07_build_main_table.py) still read the
    # legacy filename — update them in a follow-up commit once the new numbers
    # are vetted.
